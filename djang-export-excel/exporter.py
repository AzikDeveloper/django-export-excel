...
class ModelExcelExporter:
    ...
    def __init__(self, context=None, sheet_name=None, file_name=None):
        ...
        assert self.table_class is not None, f"{self.__class__}.table_class must be set"

        self._wb = xlwt.Workbook(self.style_compression)
        self._ws = self._wb.add_sheet(self.sheet_name)
    ...
    def generate(self, queryset):
        ...
        if self.include_row_number:
            self._put_cell(
                (Ex.counter, Ey.counter), (Ex.add(1), Ey.counter + 1),
                self._table.meta.header_style.xlw_style, str(_("Number"))
            )
        ...
        for column_name, column in self._table.columns.items():
            self._put_cell(
                (Ex.counter, Ey.counter), (Ex.add(1), Ey.counter + 1),
                self._table.meta.header_style.xlw_style, text=column.header_name
            )
        Ex.counter = 0
        Ey.add(1)
        ...
        for i, obj in enumerate(queryset, start=1):
            if self.include_row_number:
                self._put_cell((Ex.counter, Ey.counter), (Ex.add(1), Ey.counter + 1),
                               self._table.meta.row_style.xlw_style, text=i)

            for column_name, column in self._table.columns.items():
                self._ws.col(Ex.counter).width = column.width
                self._put_cell(
                    (Ex.counter, Ey.counter), (Ex.add(1), Ey.counter + 1),
                    self._table.meta.row_style.xlw_style, text=str(column.to_representation(obj))
                )
            Ey.add(1)
            Ex.counter = 0

    def _put_cell(self, start, end, style, text):
        self._ws.write_merge(start[1], end[1] - 1, start[0], end[0] - 1, text, style)
